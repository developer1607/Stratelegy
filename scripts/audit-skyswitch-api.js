...
const docPaths = [...new Set([...doc.matchAll(/"(\/[^"]+)":\s*\{/g)].map((m) => m[1]))].sort();
...
console.log(`Portal proxy routes registered:  ${SKYSWITCH_API_REGISTRY.length}`);
console.log('\nPBX paths in apiDocumentation.md:');
for (const p of pbxDocPaths) {
  const suffix = p.replace('/accounts/{account_id}', '');
  const covered = SKYSWITCH_API_REGISTRY.some((r) => r.skyswitch.includes(suffix));
