...
export async function migrateLegacyUserPermissionFlags() {
  ...
  for (const row of rows) {
    ...
    for (const [legacyKey, expandedKeys] of Object.entries(
    ...
    const setClause = Object.keys(patch)
      .map((k) => `\`${k}\` = 1`)
      .join(", ");
    await execute(`UPDATE user_permissions SET ${setClause} WHERE id = ?`, [
      row.id,
    ]);
    updated++;
  }
  ...
  if (updated > 0) {
    console.log(
      `[db] Expanded legacy permission flags for ${updated} user_permissions row(s)`,
    );
  }
}
