...
export async function onTicketUpdatedNotification(ticket, oldTicket, { actorEmail } = {}) {
  ...
  if (ticket.assigned_to !== oldTicket.assigned_to && isValidEmail(ticket.assigned_to)) {
    await notifyUserByEmail(
      ticket.assigned_to,
      {
        ...base,
  ...
  if (ticket.priority !== oldTicket.priority && isValidEmail(ticket.assigned_to)) {
    await notifyUserByEmail(
      ticket.assigned_to,
      {
        ...base,
...
export async function onTicketCommentNotification(ticket, comment, { actorEmail } = {}) {
  ...
  const bodyPreview =
    comment.message?.length > 120 ? `${comment.message.slice(0, 117)}…` : comment.message || '';
  ...
  if (isValidEmail(ticket.assigned_to)) recipients.add(ticket.assigned_to.toLowerCase());
  if (isValidEmail(ticket.requester_email)) recipients.add(ticket.requester_email.toLowerCase());
