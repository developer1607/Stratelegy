...
test("hasPermissionKey grants admin all permissions", () => {
  assert.equal(
    hasPermissionKey({ isAdmin: true }, "can_view_tickets_page"),
    true,
  );
  assert.equal(
    hasPermissionKey({ isAdmin: true }, "can_manage_pbx_routing"),
    true,
  );
});
...
test("hasPermissionKey denies unknown keys", () => {
  assert.equal(
    hasPermissionKey({ can_view_tickets_page: true }, "not_a_real_permission"),
    false,
  );
});
