...
export function assertTicketUpdateAllowed(
  permissions,
  existingTicket,
  updates,
) {
  ...
  const assigns =
    keys.includes("assigned_to") &&
    updates.assigned_to !== existingTicket?.assigned_to;
  const statusChanging =
    keys.includes("status") && updates.status !== existingTicket?.status;
  ...
  if (closes && !canCloseTickets(permissions)) {
    const err = new Error(
      "You do not have permission to close or resolve tickets",
    );
    err.status = 403;
    throw err;
  }
