      ...
      switch (name) {
        ...
        case "updateUserPermissions": {
          ...
          if (!userId || !updates) {
            return res
              .status(400)
              .json({ error: "Missing required fields: userId, updates" });
          }
          const permission = await updateUserPermissions({
            userId,
            userEmail,
            userName,
            updates,
        ...
        case "assignPortalRole": {
          ...
          if (!userId || !(roleId || role_id)) {
            return res
              .status(400)
              .json({ error: "Missing required fields: userId, roleId" });
          }
        ...
        case "assignTicketNumber": {
          if (req.user.role !== "admin" && !canAssignTickets(req.permissions)) {
            return res
              .status(403)
              .json({ error: "Forbidden: assign permission required" });
          }
