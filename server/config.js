...
export function assertProductionConfig() {
  ...
  if (!process.env.JWT_SECRET || config.jwtSecret === DEV_JWT_SECRET) {
    throw new Error(
      "[config] JWT_SECRET must be set to a strong value in production",
    );
  }
  ...
  if (!config.mail.enabled) {
    console.warn(
      "[config] MAIL_ENABLED is false — outbound email (invites, notifications) is disabled",
    );
  }
  if (config.skyswitch.enabled) {
    ...
    if (missing.length) {
      console.warn(
        `[config] SkySwitch enabled but missing: ${missing.join(", ")}`,
      );
    }
