...
const COMMENT_WRITABLE = new Set([
  "ticket_id",
  "message",
  "is_internal",
  "author",
...
export function validateTicketCreate(data) {
  ...
  if (d.status && !TICKET_STATUS_VALUES.includes(d.status))
    errors.push("invalid status");
  if (d.priority && !TICKET_PRIORITY_VALUES.includes(d.priority))
    errors.push("invalid priority");
  if (d.department && !TICKET_DEPARTMENT_VALUES.includes(d.department))
    errors.push("invalid department");
  if (d.source && !TICKET_SOURCE_VALUES.includes(d.source))
    errors.push("invalid source");
  if (
    d.requester_email &&
    !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d.requester_email)
  ) {
  ...
  return {
    ...
    status:
      d.status && TICKET_STATUS_VALUES.includes(d.status) ? d.status : "open",
    priority:
      d.priority && TICKET_PRIORITY_VALUES.includes(d.priority)
        ? d.priority
        : "medium",
    category: d.category,
    department: d.department || null,
    source:
      d.source && TICKET_SOURCE_VALUES.includes(d.source) ? d.source : "web",
    assigned_to: d.assigned_to || null,
    requester: d.requester ? String(d.requester).trim() : null,
    requester_email: d.requester_email
      ? String(d.requester_email).trim().toLowerCase()
      : null,
...
export function validateTicketUpdate(data) {
  ...
  if (d.description !== undefined)
    out.description = String(d.description).trim();
  ...
  if (d.requester !== undefined)
    out.requester = d.requester ? String(d.requester).trim() : null;
  if (d.requester_email !== undefined) {
    out.requester_email = d.requester_email
      ? String(d.requester_email).trim().toLowerCase()
      : null;
  }
...
export function validateCommentCreate(data) {
  ...
  return {
    ...
    author_email: d.author_email
      ? String(d.author_email).trim().toLowerCase()
      : null,
  };
}
