...
export default function OwnerSelectField({
  ...
  const { canReadEntity, hasCrmAccess, isLoading: permsLoading } = usePermissions();
  ...
  const selectValue = normalizeOwnerLabel(value) || (allowUnassigned ? OWNER_UNASSIGNED : undefined);
    ...
    <div className="space-y-2">
      ...
      <Select value={selectValue} onValueChange={handleChange} disabled={disabled || isLoading}>
        <SelectTrigger id={id} className={inputClassName}>
          <SelectValue placeholder={isLoading ? 'Loading owners...' : 'Select owner'} />
        </SelectTrigger>
        <SelectContent {...selectContentProps}>
          {allowUnassigned && <SelectItem value={OWNER_UNASSIGNED}>Unassigned</SelectItem>}
          {options.map((owner) => (
