    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Customize template content below. SMTP credentials stay in server environment variables.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {status?.from_address && (
              <span className="text-sm text-gray-600">From: {status.from_address}</span>
            )}
            {status?.smtp_host && (
          ...
          {mailStatus === 'disabled' && (
            <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-md p-3">
              ...
              is off for all users until mail is configured.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Applies when an admin creates a user or when someone completes an invite registration.
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <Label className="text-sm">Require MFA (users cannot disable)</Label>
              <p className="text-xs text-gray-500 mt-1">
                Lock email MFA on for new users — same as the admin &quot;Required&quot; toggle in
                User Management.
              </p>
            </div>
            <Switch
              ...
              disabled={
                !mailEnabled || !defaultMfaEnabled || updateSettingsMutation.isPending
              }
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a template to edit with visual, HTML, or plain-text modes.</CardDescription>
        </CardHeader>
      ...
      {activeId && (
        <EmailTemplateEditor
          ...
          onSaved={() => queryClient.invalidateQueries({ queryKey: ['emailTemplates'] })}
        />
      )}
    </div>
  );
