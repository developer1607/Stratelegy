...
export default function SubscriberDetailSheet({ domain, subscriber, open, onOpenChange, onUpdated }) {
  ...
  const [form, setForm] = useState({ email: '', department: '', site: '', notes: '', vm_pin: '' });
  ...
  const e911Q = useQuery({
    ...
    queryFn: () => pbxApi.e911Detail(String(callerId).replace(/\D/g, '').slice(-11)),
  ...
  const statusKey =
    enriched?.online_status ?? (detailQ.isFetched || detailQ.isError ? 'no_device' : null);
  const status = endpointStatusBadge(statusKey);
  const statusLabel = detailQ.isLoading && statusKey == null ? 'Loading…' : status.label;
      ...
      <SheetContent className="sm:max-w-xl overflow-y-auto">
        ...
        {canManage ? (
          <div className="mt-4 space-y-3 rounded-lg border border-gray-200 p-3 bg-gray-50">
            <h3 className="text-sm font-semibold text-gray-900">Edit extension</h3>
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <PbxFormField
                label="Voicemail PIN"
                value={form.vm_pin}
                onChange={(e) => setForm((prev) => ({ ...prev, vm_pin: e.target.value }))}
              />
            </div>
        ...
        <dl className="mt-4">
          ...
          {!canManage ? <DetailRow label="Email" value={enriched.email_address} /> : null}
          ...
          {!canManage ? (
            <DetailRow label="Department" value={enriched.department || enriched.group} />
          ) : null}
          <DetailRow label="Transport" value={enriched.transport} />
          <DetailRow label="Geo node" value={enriched.geo_node} />
          <DetailRow label="WAN IP" value={enriched.wan_ip || enriched.wan_lan} />
          ...
          <DetailRow
            label="Features"
            value={(enriched.features || []).length ? enriched.features.join(', ') : null}
          />
          {!canManage ? <DetailRow label="Notes" value={enriched.notes} /> : null}
        </dl>

        {canManage && !macAddress ? (
          <div className="mt-4">
            <AddPhoneAction domain={domain} extension={enriched.user} onSuccess={refreshDetail} />
          </div>
        ) : null}
          ...
          <section className="mt-6">
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {canManage ? (
                  <DeletePhoneAction domain={domain} macAddress={macAddress} onSuccess={refreshDetail} />
                ) : null}
              </div>
            </div>
            {phoneQ.isLoading && !phone ? (
              ...
              <div className="border rounded-lg p-3 bg-gray-50 space-y-1 text-sm">
                <p>
                  <span className="text-gray-500">Model:</span> {phone.model || '—'}
                </p>
                <p>
                  <span className="text-gray-500">Primary line:</span> {phone.primary_line || '—'}
                </p>
                <p>
                  <span className="text-gray-500">Device:</span> {phone.primary_device || '—'}
                </p>
                <p>
                  <span className="text-gray-500">Transport:</span> {phone.transport || '—'}
                </p>
                <p>
                  <span className="text-gray-500">User agent:</span> {phone.user_agent || '—'}
                </p>
                <p>
                  <span className="text-gray-500">Registered at:</span> {phone.registration_time || '—'}
                </p>
              </div>
            ) : null}
          </section>
