...
export default function PbxShell({
  ...
  const showDomainsLink =
    canViewPbxDomains(permissions) && canAccessPage("PBXDomains");
    ...
    <div className="p-4 sm:p-8 space-y-6">
      ...
      {requiresDomain && error && canListDomains && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
          ...
          <p className="text-sm mt-1">{displayError(error, "Load failed.")}</p>
        </div>
      )}
      ...
      {requiresDomain && !isLoading && !domain && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-900 text-sm">
          {canListDomains ? (
            ...
            <>
              Add a <span className="font-mono">?domain=your.domain</span> query
              parameter to the URL, or ask an admin for Domains access to browse
              available domains.
            </>
          )}
        </div>
    ...
    <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
      ...
      {error.data?.code === "pbx_domain_scope_required" && (
        <p className="text-sm mt-2">
          Use the domain-specific live views under PBX, or ask an admin for
          account-wide report access.
        </p>
      )}
              ...
              return (
                ...
                >
                  {columns.map((col) => (
                    <td
                      key={col.key}
                      className="px-4 py-3 text-gray-800 align-middle"
                    >
