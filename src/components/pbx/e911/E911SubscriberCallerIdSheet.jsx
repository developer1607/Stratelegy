      ...
      <SheetContent className="sm:max-w-md">
        <SheetHeader>
          ...
          <SheetDescription>
            ...
            <code className="text-xs">callid_emgr</code>. Civic address
            provisioning uses the phone-number workflow when Telco E911 is
            enabled.
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600 space-y-1">
            <p>
              ...
              <span className="font-medium text-gray-900">
                {subscriber.caller_id || "—"}
              </span>
            </p>
            <p>
              Current PBX 911 CID:{" "}
              <span className="font-medium text-gray-900">
                {subscriber.e911_caller_id || "—"}
              </span>
            </p>
            ...
            {profile?.addresses?.supported === false ? (
              <p className="text-amber-700">
                PBX civic addresses API is not available on this portal.
              </p>
            ) : null}
            {showV2Mismatch ? (
              <p className="text-amber-700">
                v2 profile differs from v1. Saving updates v1{" "}
                <code>callid_emgr</code>, which is used for routing on this
                portal.
              </p>
            ) : null}
          </div>
