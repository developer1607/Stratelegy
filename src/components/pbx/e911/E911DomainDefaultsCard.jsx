...
export default function E911DomainDefaultsCard({
  domain,
  defaults,
  capabilities,
  onSuccess,
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              PBX domain-level defaults from{" "}
              <code className="text-xs">callid_emgr</code> /{" "}
              <code className="text-xs">callid_nmbr</code>. Use the emergency
              pool tab to manage routable 911 numbers when extensions use{" "}
        ...
        {!capabilities?.domainDefaultsWrite ? (
          <p className="text-xs text-amber-700">
            This portal may not accept domain default updates via API. Emergency
            pool management remains available.
          </p>
        ) : null}
      </div>
