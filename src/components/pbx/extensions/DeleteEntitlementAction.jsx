...
export default function DeleteEntitlementAction({ entitlement, onSuccess }) {
  const mutation = useMutation({
    ...
    onError: (err) => toast.error(err?.message || 'Failed to remove entitlement'),
  });

  if (!entitlement?.id) return null;

  const label = [entitlement.subscriber, entitlement.offering?.name || entitlement.offering_name]
    .filter(Boolean)
    .join(' — ');
