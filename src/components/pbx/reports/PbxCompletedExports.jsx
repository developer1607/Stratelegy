...
export default function PbxCompletedExports({
  ...
  const { data, isLoading, error, refetch, isFetching } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const hasQueued = list.some((item) =>
        /queued|pending/i.test(String(item.status || "")),
      );
  ...
  const { rows, statusOptions, hasQueued, currentPage } = useMemo(() => {
    ...
    const typeFiltered = reportTypeMatch
      ? list.filter((row) =>
          reportTypeMatch.test(String(row.report_type || "")),
        )
      : list;
    ...
    return {
      ...
      hasQueued: typeFiltered.some((row) =>
        /queued|pending/i.test(String(row.status || "")),
      ),
      rows: filtered,
        ...
        <>
          ...
          {hasActiveFilters && (
            <p className="text-xs text-amber-700">
              Search and status filters apply to the current page of exports
              only.
            </p>
          )}
          ...
          {rows.length > 0 && (
            <div className="flex items-center justify-between text-sm text-gray-600">
              <span>
                {rows.length} matching export(s) on page {currentPage}
              </span>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  disabled={
                    (data?.data?.length || 0) < (Number(data?.per_page) || 25)
                  }
