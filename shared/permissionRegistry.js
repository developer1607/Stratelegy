...
import { PBX_OPERATIONAL_REPORT_PAGES } from "./pbxReportPages.js";
...
export const PERMISSION_DEFS = [
  ...
  {
    key: "can_manage_pbx_endpoints",
    label: "Manage hub users",
    module: "pbx",
    section: "Actions",
  },
];
...
const CRM_KEYS = PERMISSION_DEFS.filter(
  (d) => d.module === "crm" && !d.master,
).map((d) => d.key);
const SUPPORT_KEYS = PERMISSION_DEFS.filter(
  (d) => d.module === "support" && !d.master,
).map((d) => d.key);
const PBX_KEYS = PERMISSION_DEFS.filter(
  (d) => d.module === "pbx" && !d.master,
).map((d) => d.key);
...
export const TICKET_ACTION_KEYS = Object.fromEntries(
  PERMISSION_DEFS.filter((d) => d.ticketAction).map((d) => [
    d.ticketAction,
    d.key,
  ]),
);
...
export const LEGACY_PERMISSION_EXPAND = {
  ...
  can_manage_contacts: [
    "can_view_contacts_page",
    "can_view_contacts",
    "can_manage_contacts",
  ],
  can_manage_accounts: [
    "can_view_accounts_page",
    "can_view_accounts",
    "can_manage_accounts",
  ],
  can_manage_leads: [
...
export function buildPermissionGroupsForUI() {
  const modules = [
    { id: "crm", label: "CRM", icon: "crm", masterKey: "can_access_crm" },
    {
      ...
      label: "Support Desk",
      icon: "support",
      masterKey: "can_access_support",
    },
    { id: "pbx", label: "PBX", icon: "pbx", masterKey: "can_access_pbx" },
  ];

  return modules.map((mod) => {
    const defs = PERMISSION_DEFS.filter(
      (d) => d.module === mod.id && !d.master,
    );
